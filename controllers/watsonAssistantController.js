...
let sendMessageAnalysis = async (req, res) => {
  try {
    let responseTranslation = await axios.post(`${process.env.URL_API}/translate`, {
      text: req.body.text,
    });
    let translation = responseTranslation.data;
    console.log(JSON.stringify(translation, null, 2));

    let responseEmotion = await axios.post(`${process.env.URL_API}/analyze/emotion`, {
      text: translation.translations[0].translation,
    });
    let emotions = responseEmotion.data;
    console.log(JSON.stringify(emotions, null, 2));

    let emotionObject = emotions.utterances_tone[0].tones.reduce(function (
      prev,
      current
    ) {
